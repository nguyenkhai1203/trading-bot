...
## 8. BTC Macro Signal (BMS) & Market Filters
The BMS acts as a "Supreme Filter" to provide global market context.

### 1. The Composite MTF Score (0.0 - 1.0)
Calculated by `BTCAnalyzer` using a Multi-Timeframe (MTF) approach:
- **Aggregation**: **1h (30%)**, **4h (40%)**, and **1d (30%)**.
- **Normalization**: All sub-scores and the final composite are scaled to `0.0 - 1.0` (0.5 = Neutral).
- **Sub-components**: Trend (EMA relations), Momentum (RSI/MACD), Volatility (ATR Panic), and Dominance.

### 2. Symmetrical Veto & Active Shield
- **🟢 GREEN (> 0.7)**: Strong Bullish. **Vetoes SHORT entries**. Boosts LONG confidence.
- **🟡 YELLOW (0.3 - 0.7)**: Neutral Market. Normal weighted logic applies.
- **🔴 RED (< 0.3)**: Bearish Panic. **Vetoes LONG entries**. Prioritizes SHORTS.
- **Active Shield**: Bot proactively closes open positions if the BMS zone shifts into the "Veto Zone" for that trade (e.g., closing LONG if BMS enters RED).

### 3. Two-Tier Optimization Flow
- **Loop A (Internal BTC)**: Optimizes internal sub-weights (Trend, Mom, etc.) to maximize future return correlation.
- **Loop B (Global Strategy)**: Integrated into `StrategyAnalyzer` Grid Search to find optimal $W_{BTC} / W_{Alt}$ weights for every specific pair and timeframe.
